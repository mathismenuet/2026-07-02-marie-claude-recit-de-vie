...
import { ACCESS_CODE } from '../data/siteConfig';
...
export default function AccessGate({ onUnlock }: Props) {
  ...
  useEffect(() => {
    if (localStorage.getItem('marie-claude-access') === 'granted') {
      onUnlock();
    }
  }, [onUnlock]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (code.trim().toUpperCase() === ACCESS_CODE) {
      localStorage.setItem('marie-claude-access', 'granted');
      setUnlocked(true);
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-dark-green transition-opacity duration-700 ease-in-out ${unlocked ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      >
        ...
        <h1 className="text-3xl sm:text-4xl font-normal text-white mb-4 tracking-tight" style={{ fontFamily: '"Neue Haas Grotesk Display Pro 55 Roman", sans-serif' }}>Marie-Claude nous raconte</h1>
        <p className="text-white/70 text-sm sm:text-base leading-relaxed mb-8 font-inter">
          (sa vie, ses histoires, sa liberté) — Entrez le code pour accéder à la page.
        </p>
